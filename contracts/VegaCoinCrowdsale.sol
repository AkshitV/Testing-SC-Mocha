...
contract VegaCoinCrowdsale is Pausable, StandardToken {
    ...
    uint256 constant public TEAM_SUPPLY = 13520000e18;       //26% of675m = 175.5M
    uint256 constant public ALB_SUPPLY = 19800000e18;        //4% of 675M = 27M
    ...
    address public teamFund;   //  = 0x833cE4a216331af41961D01c5331e03EBfffb888
    address public albFund ;    // = 0xCd637E775C8ABC095BEc957807f6E7E6628931C2
    ...
    constructor() public{
        ...
        roundOne.roundStartDate = 1532170800;        //1st -Aug   
        roundOne.roundEndDate = 1532257200;          //14th -Aug
       ...
       //Round Two
        ...
        roundTwo.roundStartDate = 1532343600;        //15th Aug
        roundTwo.roundEndDate = 1532430000;          //29th Sept
       ...
       //Round Three
        ...
        roundThree.roundStartDate = 1532516400;        //30th Oct
        roundThree.roundEndDate = 1532602800;          //13th Sept
        ...
        roundFour.roundStartDate = 1532689200;        //14th Sept
        roundFour.roundEndDate = 1532775600;          //29th Sept
        ...
        roundFive.roundStartDate = 1531979961;        //30th Sept
        roundFive.roundEndDate = 1532088000;          //14th Oct
        ...
        roundSix.roundStartDate = 1531979961;        //15th Oct
        roundSix.roundEndDate = 1532088000;          //30th Oct
    ...
    modifier saleIsFinished(){
        require(now > EndDate.add(30 days)); // 155555 = end date of round6
        _;
    }
    
    modifier canTransfer(address sender){
        if (sender == teamFund) 
            require(now > (EndDate.add(365 days)));
        else if (sender == albFund) 
            require(now > (EndDate.add(365 days)));
        else  
            require(now > (EndDate.add(30 days)));    
        _;
    }
    ...
    //internal //have done view
    function calculateBonus(uint256 _tok) public view returns (uint256) {
    ...
    function () external  payable {//saleIsonn whenNotPaused
        ...
        uint256 tokens;
        uint256 bonustokens;
        tokens = msg.value.mul(currentAAAPerEther);
    ...
    function refundEther(address _yourAddresss) external {//internal{
        //convert value
    ...
    // Merge End Ico Sale into endCurrent Round Sale.
  ...
  function sendBounty(address to, uint256 tokens) external returns (bool) {
    ...
    function setDate(uint32 startDate, uint32 endDate) external onlyOwner returns (bool) {
        StartDate = startDate;
        EndDate = endDate;
        return true;
    }

    function setBountyAddress(address _add) external onlyOwner returns (bool) {
        bountyAddress = _add;
        return true;
    }
    function setTeamAddress(address _add) external onlyOwner returns (bool) {
        teamFund = _add;
        return true;
    }
    function setAlbAddress(address _add) external onlyOwner returns (bool) {
        albFund = _add;
        return true;
    }
